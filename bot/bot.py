...
class TelegramBotHandler:
    ...
    # ── Bot-code web auth ─────────────────────────────────────────────────
    async def _claim_auth_code(self, update: Update, code: str) -> None:
        """Claim a web/mobile login code via the backend API."""
        user = update.effective_user
        if not user:
            return

        if not API_BASE_URL or not BOT_TOKEN:
            logger.error("_claim_auth_code: API_BASE_URL or BOT_TOKEN not set")
            await update.message.reply_text("❌ Server sozlanmagan. Admin bilan bog'laning.")
            return

        try:
            async with httpx.AsyncClient(timeout=10) as client:
                res = await client.post(
                    f"{API_BASE_URL}/api/auth/bot-claim-code",
                    json={
                        "code":        code,
                        "telegram_id": user.id,
                ...
                if res.status_code == 404:
                    await update.message.reply_text("❌ Kod topilmadi yoki muddati o'tgan.")
                    return
                if res.status_code == 410:
                    await update.message.reply_text("⏰ Bu kod muddati o'tgan. Qayta urinib ko'ring.")
                    return
                if res.status_code != 200:
                    logger.error(f"_claim_auth_code: backend returned {res.status_code}: {res.text}")
                    await update.message.reply_text("❌ Xatolik yuz berdi. Qayta urinib ko'ring.")
                    return

        except Exception as e:
            logger.error(f"_claim_auth_code error: {e}")
            await update.message.reply_text("❌ Server bilan bog'lanishda xatolik.")
            return

        await update.message.reply_text(
            f"✅ Muvaffaqiyatli kirildi, {user.first_name}!\n\n"
            "Ilovaga qayting — kirish avtomatik bo'ladi. 🚀"
        )

    # ── /start — regular or deep-link pay_{order_id} / auth_{code} ──────────
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if args and args[0].startswith("auth_"):
            ...
            await self._claim_auth_code(update, code)
            return
